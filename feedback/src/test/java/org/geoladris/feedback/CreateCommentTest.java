...
import org.geoladris.feedback.CannotSendMailException;
import org.geoladris.feedback.CommentInfo;
import org.geoladris.feedback.Feedback;
import org.geoladris.feedback.FeedbackPersistence;
import org.geoladris.feedback.Mailer;
import org.geoladris.feedback.MissingArgumentException;
...
public class CreateCommentTest {
  ...
  @Test
  public void testMissingParameters() throws CannotSendMailException, PersistenceException {
  ...
  private void testMandatoryParameter(String geom, String comment, String email, String layerName)
    ...
    try {
      feedback.insertNew(geom, comment, email, layerName, validDate, language, title,
          verificationMessage);
      fail();
  ...
  public void testDateCanBeNull() throws Exception {
    feedback = new Feedback(mock(FeedbackPersistence.class), mock(Mailer.class));
    feedback.insertNew(validGeometry, validComment, validEmail, validDate, null, language, title,
        verificationMessage);
  }
  ...
  public void testInvalidInsertNoMail() throws Exception {
    ...
    doThrow(new PersistenceException("", null)).when(persistence).insert(anyString(), anyString(),
        anyString(), anyString(), anyString(), anyString(), anyString(), anyString());
    Mailer mailer = mock(Mailer.class);
    feedback = new Feedback(persistence, mailer);

    try {
      feedback.insertNew(validGeometry, validComment, validEmail, validLayer, validDate, "ca",
          title, verificationMessage);
      fail();
    } catch (Exception e) {
    }
    verify(mailer, never()).sendVerificationMail(anyString(), anyString(), anyString(), anyString(),
        anyString());
  }
  ...
  public void testInsert() throws Exception {
    ...
    feedback.insertNew(validGeometry, validComment, validEmail, validLayer, validDate, language,
        title, verificationMessage);
    verify(persistence, times(1)).insert(eq(validGeometry), eq("900913"), eq(validComment),
        eq(validEmail), eq(validLayer), eq(validDate), anyString(), anyString());
  }

  @Test
  public void testDifferentVerificationCodes() throws Exception {
    feedback = new Feedback(mock(FeedbackPersistence.class), mock(Mailer.class));
    assertTrue(feedback.insertNew(validGeometry, validComment, validEmail, validLayer, validDate,
        language, title, verificationMessage) != feedback.insertNew("POINT(1 1)", validComment,
            validEmail, validLayer, validDate, language, title, verificationMessage));
  }
  ...
  public void testNotifyAuthors() throws Exception {
    ...
    when(persistence.getValidatedToNotifyInfo())
        .thenReturn(new CommentInfo[] {new CommentInfo(1, "a@b.com", "100", language)});
    Mailer mailer = mock(Mailer.class);
    feedback = new Feedback(persistence, mailer);
    feedback.notifyValidated(config);

    verify(mailer).sendValidatedMail("a@b.com", "100", validationMessage, title);
  ...
  public void testNotifyAuthorsMailError() throws Exception {
    ...
    Mailer mailer = mock(Mailer.class);
    doThrow(new MessagingException()).when(mailer).sendValidatedMail("c@d.com", "101",
