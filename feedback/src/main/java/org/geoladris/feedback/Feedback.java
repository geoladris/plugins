...
public class Feedback {
  ...
  public String insertNew(String geom, String comment, String email, String layerName,
      String layerDate, String linkLanguage, String mailTitle, String verificationMessage)
    ...
    /*
     * non unique verification code, but these are valid only a period of time so collisions are
     * very rare
     */
    String verificationCode = Integer.toString((geom + comment + email).hashCode());
    persistence.insert(geom, srid, comment, email, layerName, layerDate, verificationCode,
        linkLanguage);
    try {
      mailInfo.sendVerificationMail(linkLanguage, mailTitle, verificationMessage, email,
          verificationCode);
  ...
  public void verify(String verificationCode) throws VerificationCodeNotFoundException,
      PersistenceException, AddressException, MessagingException {
  ...
  public void notifyValidated(Config config) throws PersistenceException {
    ...
    for (CommentInfo entry : entries) {
      try {
        ...
        mailInfo.sendValidatedMail(entry.getMail(), entry.getVerificationCode(),
