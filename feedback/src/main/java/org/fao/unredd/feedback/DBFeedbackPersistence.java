...
public class DBFeedbackPersistence implements FeedbackPersistence {
    ...
    DBUtils.processConnection("unredd-portal", new DBUtils.DBProcessor() {
      ...
      public void process(Connection connection) throws SQLException {
        PreparedStatement statement = connection.prepareStatement(
            "INSERT INTO " + tableName + "(geometry, comment, date, email, layer_name, "
                + "layer_date, verification_code, language, state) " + "VALUES"
                + "(ST_GeomFromText(?, ?), ?, ?, ?, ?, ?, ?, ?, " + NEW + ")");
    ...
    DBUtils.processConnection("unredd-portal", new DBUtils.DBProcessor() {
      ...
      public void process(Connection connection) throws SQLException {
        PreparedStatement statement = connection
            .prepareStatement("DELETE FROM " + tableName + " WHERE verification_code IS NOT NULL"
    ...
    return DBUtils.processConnection("unredd-portal", new DBUtils.ReturningDBProcessor<Boolean>() {
      ...
      public Boolean process(Connection connection) throws SQLException {
        PreparedStatement statement = connection.prepareStatement("SELECT count(*) FROM "
            + tableName + " WHERE verification_code = ? AND state = " + NEW);
    ...
    DBUtils.processConnection("unredd-portal", new DBUtils.DBProcessor() {
      ...
      public void process(Connection connection) throws SQLException {
        PreparedStatement statement = connection.prepareStatement("UPDATE " + tableName
            + " SET state = " + VERIFIED + " WHERE verification_code = ? AND state = " + NEW);
        statement.setString(1, verificationCode);
        statement.execute();
    ...
    DBUtils.processConnection("unredd-portal", new DBUtils.DBProcessor() {
      ...
      public void process(Connection connection) throws SQLException {
        PreparedStatement statement =
            connection.prepareStatement("SELECT id, email, verification_code, language FROM "
                + tableName + " WHERE state=" + VALIDATED);
        ResultSet result = statement.executeQuery();
        while (result.next()) {
          ret.add(new CommentInfo(result.getInt("id"), result.getString("email"),
              result.getString("verification_code"), result.getString("language")));
        }
        result.close();
        statement.close();
      }
  ...
  public void setNotified(final int id) throws PersistenceException {
    DBUtils.processConnection("unredd-portal", new DBUtils.DBProcessor() {
      ...
      public void process(Connection connection) throws SQLException {
        PreparedStatement statement = connection
            .prepareStatement("UPDATE " + tableName + " SET state = " + NOTIFIED + " WHERE id = ?");
